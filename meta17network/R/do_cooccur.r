...
#                        (cooccurs_bypop_res$obs_cooccur / cooccurs_bypop_res$exp_cooccur))
    ...
    colnames(toPlot_all) <- c("pair", "sp1", "sp2", "obs", "obs_div_exp")
    colnames(toPlot_pop_aggr) <- colnames(toPlot_all)
    ...
    toPlot_pop4 <- toPlot_pop3
    toPlot_pop3[rownames(toPlot_pop_aggr), ] <- toPlot_pop_aggr
    toPlot_pop4[rownames(toPlot_all), ] <- toPlot_all
        
    #toPlot_pop2 <- toPlot_pop3
    #toPlot_pop2[, c("obs", "obs_div_exp")] <- -1
    #toPlot_pop2[rownames(toPlot_pop), ] <- toPlot_pop
    
    cooccur_res_list <- list("populations_aggregated" = toPlot_pop3, 
                             #"populations" = toPlot_pop2,
                             "full_data" = toPlot_pop4)
    #saveRDS(cooccur_res_list, file = file.path(dirs$fits, "cooccur_res_list.rds"))

    res <- list("cooccur_population_aggr" = cooc_bypop_aggr, 
                #"cooccur_population" = cooccurs_bypop, 
